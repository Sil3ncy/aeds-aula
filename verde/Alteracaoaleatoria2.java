            ...
            while(x){
                ...
                if(frase.equals("FIM")){ //verificação da string inserida para encerramento do codigo ou continuação
                    ...
                    for(int i = 0; i<frase.length(); i++){//inicio do loop que percorrerá a string realizando as alterações
                        if(chararray[i] == aleatorio1 && chararray[i] >= 'a' && chararray[i]<= 'z'){ //checagem pela primeira letra e certificação que ela é minúscula 
                            chararray[i] = aleatorio2;
                        }

                        else{
                            if(chararray[i] == aleatorio2 && chararray[i] >= 'a' && chararray[i]<= 'z'){ //checagem e troca da segunda letra e verificação para minúsculas 
